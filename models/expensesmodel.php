...
class ExpensesModel extends Model implements IModel{
    ...
    public function getTotalAmountThisMonth($userid){
        try{
            $year = date('Y');
            $month = date('m');
            //aqui se agrego un error
            error_log("getTotalAmountThisMonth -> userid: $userid, year: $year, month: $month");
            ...
            error_log("getTotalAmountThisMonth -> total: $total");
            
            return $total;

        }catch(PDOException $e){
            error_log('ExpensesModel::getTotalAmountThisMonth -> ' . $e->getMessage());
            return NULL;
        }
    }
    ...
    public function getMaxExpensesThisMonth($userid){
        try{
            $year = date('Y');
            $month = date('m');
            error_log("getMaxExpensesThisMonth -> userid: $userid, year: $year, month: $month");
            ...
            $total = $query->fetch(PDO::FETCH_ASSOC)['total'];
            if($total == NULL) $total = 0;
            error_log("getMaxExpensesThisMonth -> total: $total");
            
            return $total;

        }catch(PDOException $e){
            error_log('ExpensesModel::getMaxExpensesThisMonth -> ' . $e->getMessage());
